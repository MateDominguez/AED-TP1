...
\begin{proc}{sumaDeHabitantes}{\In menoresDeCiudades : \TLista{Ciudad}, \In mayoresDeCiudades : \TLista{Ciudad}}{\TLista{Ciudad}}
	\requiere{noHayRepetidos(menoresDeCiudades) \wedge noHayRepetidos(mayoresDeCiudades)}
	\requiere{|menoresDeCiudades| = |mayoresDeCiudades|}
	\asegura{|res| = |s_1|}
	\asegura{noHayRepetidos(res)}
	\asegura{(\forall i,j : \ent)(0 \leq i,j < |menoresDeCiudades| \yLuego menoresDeCiudades[i][0] = mayoresDeCiudades[j][0] \implicaLuego (res[i][1] = sumaHabitantes(menoresDeCiudades[i], mayoresDeCiudades[j])))}
\end{proc}
...
\begin{proc}{cantidadCaminosNSaltos}{\Inout s : \TLista{\TLista{\ent}}, \In n : \ent}{\TLista{\TLista{\ent}}}
	\requiere{s = s_0}
	\requiere{esCuadrada(s_0)}
	\requiere{(\forall i,j : \ent)(0 \leq i,j < |s_0|\implicaLuego(s_0[i][j] = s_0[j][i]\wedge(s_0[i][j] = 0 \vee s_0[i][j] = 1)))}
	\requiere{(\forall i,j : \ent)(((0 \leq i,j < |s_0|)\wedge(i = j))\implicaLuego(s_0[i][j] = 0))}
	\requiere{n \geq 1}
	\asegura{|s| = |s_0|}
	\asegura{esCuadrada(S)}
	\asegura{(\forall i,j : \ent)(0 \leq i,j < |s|\implicaLuego(s[i][j] = s[j][i]))}
	\asegura{(\exists t : \TLista{\TLista{\TLista{\ent}}})(|t| = n \wedge t[0] = s_0 \wedge(\forall i : \ent)(0 \leq i < |s_0| - 1 \implicaLuego (APorBEsC(t[i], s_0, t[i+1]) \\ \wedge s = t[n-1])))}
\end{proc}

\vspace{0.3cm}

\pred{APorBEsC}{A : \TLista{\TLista{\ent}}, B : \TLista{\TLista{\ent}}, C : \TLista{\TLista{\ent}}}{(\forall i,j : \ent)(0 \leq i,j < |C| \implicaLuego (C[i][j] = \sum\limits_{i=0}^{|C| - 1} A[i][j]*B[j][i]))} 
