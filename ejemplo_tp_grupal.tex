...
\begin{proc}{sumaDeHabitantes}{\In menoresDeCiudades : \TLista{Ciudad}, \In mayoresDeCiudades : \TLista{Ciudad}}{\TLista{Ciudad}}
	\requiere{noHayCiudadesRepetidas(menoresDeCiudades) \wedge noHayCiudadesRepetidas(mayoresDeCiudades)}
	\requiere{mismasCiudades(menoresDeCiudades, mayoresDeCiudades)}
	\asegura{|res| = |s_1|}
	\asegura{noHayCiudadesRepetidas(res)}
...
\begin{proc}{cantidadCaminosNSaltos}{\Inout s : \TLista{\TLista{\ent}}, \In n : \ent}{}
	\requiere{s = s_0}
	...
	\asegura{(\exists t : \TLista{\TLista{\TLista{\ent}}})(|t| = n \wedge t[0] = s_0 \wedge (\forall i,j : \ent)(0 \leq i,j < |t| \implicaLuego |t[i]|=|t[j]| \wedge esCuadrada(t[i])) \yLuego \\ (\forall i : \ent)(0 \leq i < |s_0| - 1 \implicaLuego (APorBEsC(t[i], s_0, t[i+1]))) \wedge s = t[n-1])}
\end{proc}

\vspace{0.3cm}

\pred{APorBEsC}{A : \TLista{\TLista{\ent}}, B : \TLista{\TLista{\ent}}, C : \TLista{\TLista{\ent}}}{(\forall i,j : \ent)(0 \leq i,j < |C| - 1\implicaLuego (C[i][j] = \sum\limits_{k=0}^{|C| - 1} A[i][k]*B[k][j]))}
